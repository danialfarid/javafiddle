...
$cheeta.resource = function (config) {
	...
	var ResourceClass = function (obj) {
		...
		this.$resolveUrl = function () {
			return config.url.replace('(:\\w+)', function (m) {
				var val = this[m.substring(1)];
				return val ? val : '';
			});
		};
		...
		this.$post = this.$create = function (fn) {
			this.$xhr().open('POST', this.$resolveUrl(), config.async || true).send(this)
				.after(function (x, data) {
					$cheeta.copy(data, this);
					fn.call(this, data);
				});
			return this;
		};

		this.$put = this.$update = function (fn) {
			this.$xhr().open('PUT', this.$resolveUrl(), config.async || true).send(this)
				.after(function (x, data) {
					fn.call(this, data);
				});
			return this;
		...
		this.$remove = function (fn) {
			this.$xhr().open('DELETE', this.$resolveUrl(), config.async || true).send()
				.after(function (x, data) {
					fn.call(this, data);
				});
			return this;
		...
		this.$get = function (fn) {
			this.$xhr().open('GET', this.$resolveUrl(), config.async || true).send()
				.after(function (x, data) {
					$cheeta.copy(data, this);
					fn.call(this, data);
				});
		...
		this.$query = function (params, fn) {
			return ResourceClass.$query(params, this).after(fn);
		};
	};

	ResourceClass.$query = function (params, sample) {
		var resp = [], after;
		resp.after = function (fn) {
			after = fn;
		};

		this.$xhr().open('GET', new $cheeta.url(sample.$resolveUrl(config.url)).params(params).toStr(),
			config.async || true).send().after(function (x, data) {
				for (var i = 0; i < data.length; i++) {
					resp[i] = new ResourceClass(data[i]);
				}
				after.call(this, resp);
			});
		return resp;
	};
